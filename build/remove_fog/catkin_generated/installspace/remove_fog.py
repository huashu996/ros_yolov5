...
class SubscribeAndPublish:
		...
		def deHaze(self,img, r=400, eps=0.005, w=0.95, maxV1=0.10, bGamma=True): #r=81, eps=0.001, w=0.95, maxV1=0.80, bGamma=False
				
				Y = np.zeros(img.shape)
				cv2.imwrite("/home/cxl/ros_yolov5/src/datasets/result/2.png",Y)
				V1,A = self.getV1(img, r, eps, w, maxV1)        #得到遮罩图像和大气光照
				for k in range(3):
					Y[:,:,k] = (img[:,:,k]-V1)/(1-V1/A)      #颜色校正
				Y = np.clip(Y, 0, 1)
				m1 = Y*255
				m1 = np.clip(m1,a_min=0,a_max=255) #先归一化到0-2500消除负数
				cv2.imwrite("/home/cxl/ros_yolov5/src/datasets/result/1.png",m1)
				#if bGamma:
				Y = Y**(np.log(0.3)/np.log(Y.mean()))    #gamma校正, 越大越亮  0.3
				m2 = Y*255
				m2 = np.clip(m2,a_min=0,a_max=255) #先归一化到0-2500消除负数
				cv2.imwrite("/home/cxl/ros_yolov5/src/datasets/result/1.png",m2)
				Y = self.imgBrightness(Y, 2.8, -0.15)   #第一个值是亮度，第二个是对比度 2.8 -0.15
				m3 = Y*255
				m3 = np.clip(m3,a_min=0,a_max=255) #先归一化到0-2500消除负数
				cv2.imwrite("/home/cxl/ros_yolov5/src/datasets/result/1.png",m3)
