...
class SubscribeAndPublish:
		...
		def deHaze(self,img, r=81, eps=0.001, w=0.95, maxV1=0.80, bGamma=False):
				...
				Y = np.clip(Y, 0, 1)
				#if bGamma:
				Y = Y**(np.log(0.3)/np.log(Y.mean()))    #gamma校正, 越大越亮
				Y = self.imgBrightness(Y, 2.8, -0.22)   #第一个值是亮度，第二个是对比度
				return Y
